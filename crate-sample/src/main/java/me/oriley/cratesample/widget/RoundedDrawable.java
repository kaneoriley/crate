...
import me.oriley.cratesample.widget.RoundedImageView.Corner;
...
public class RoundedDrawable extends Drawable {
    ...
    public RoundedDrawable(@NonNull Bitmap bitmap) {
        ...
        mBitmapRect.set(0, 0, mIntrinsicWidth, mIntrinsicHeight);
        ...
        mBorderPaint.setColor(mBorderColors.getColorForState(getState(), DEFAULT_BORDER_COLOR));
    ...
    public boolean isStateful() {
        return mBorderColors.isStateful();
    }

    @Override
    protected boolean onStateChange(@Nullable int[] state) {
        int newColor = mBorderColors.getColorForState(state, 0);
    ...
    private void updateShaderMatrix() {
        ...
        switch (mScaleType) {
            case CENTER:
                ...
                mShaderMatrix.reset();
                mShaderMatrix.setTranslate((int) ((mBorderRect.width() - mIntrinsicWidth) * 0.5f + 0.5f),
                        (int) ((mBorderRect.height() - mIntrinsicHeight) * 0.5f + 0.5f));
                break;
            ...
            case CENTER_CROP:
                ...
                if (mIntrinsicWidth * mBorderRect.height() > mBorderRect.width() * mIntrinsicHeight) {
                    scale = mBorderRect.height() / (float) mIntrinsicHeight;
                    dx = (mBorderRect.width() - mIntrinsicWidth * scale) * 0.5f;
                } else {
                    scale = mBorderRect.width() / (float) mIntrinsicWidth;
                    dy = (mBorderRect.height() - mIntrinsicHeight * scale) * 0.5f;
                }
            ...
            case CENTER_INSIDE:
                ...
                if (mIntrinsicWidth <= mBounds.width() && mIntrinsicHeight <= mBounds.height()) {
                    scale = 1.0f;
                } else {
                    scale = Math.min(mBounds.width() / (float) mIntrinsicWidth,
                            mBounds.height() / (float) mIntrinsicHeight);
                }

                dx = (int) ((mBounds.width() - mIntrinsicWidth * scale) * 0.5f + 0.5f);
                dy = (int) ((mBounds.height() - mIntrinsicHeight * scale) * 0.5f + 0.5f);
    ...
    public void draw(@NonNull Canvas canvas) {
        if (mRebuildShader) {
            BitmapShader bitmapShader = new BitmapShader(mSourceBitmap, mTileModeX, mTileModeY);
    ...
    public RoundedDrawable setBorderColor(@Nullable ColorStateList colors) {
        mBorderColors = colors != null ? colors : ColorStateList.valueOf(0);
        mBorderPaint.setColor(mBorderColors.getColorForState(getState(), DEFAULT_BORDER_COLOR));
        return this;
    }
